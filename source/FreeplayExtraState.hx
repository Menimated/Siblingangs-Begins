...
{
	var songs:Array<SongMetadataExtra> = [];
	...
	override function create()
	{
		...
		addSong('asking', 4, 'hinata', FlxColor.YELLOW);
		}
		addSong('UwU', 4, 'kazashi', FlxColor.PINK);
		if(ClientPrefs.weekMeni){
		addSong('Familiar', 4, 'menimated', FlxColor.GRAY);
		addSong('Noticed', 4, 'hinata', FlxColor.YELLOW);
		}
		if(ClientPrefs.weekDNB){
		addSong('Best friends', 4, 'kazashi', FlxColor.GREEN);
		}
		addSong('Cuteness', 4, 'hiroko', FlxColor.PINK);
		addSong('Zuz moogus', 4, 'zukiAmongUs', FlxColor.ORANGE);
		if(ClientPrefs.weekDNB){
		addSong('Legend advance', 4, 'tristan', FlxColor.RED);
		}
		if(ClientPrefs.smolshi)
			addSong('kawa nyaa', 4, 'smolshi', FlxColor.PINK);
	    if(ClientPrefs.smolshi2)
			addSong('Neko neko', 4, 'smolshi-pixel', FlxColor.PINK);

		if(ClientPrefs.cheatersneverwin)
			addSong('Cheaters never win', 4, 'cheating', FlxColor.GREEN);

		if(ClientPrefs.unsurvivable)
			addSong('unsurvivable', 7, 'trueex', FlxColor.RED);
		if(ClientPrefs.unsurvivable)
		    addSong('defeat', 4, 'trueex', FlxColor.BLACK);

		if(ClientPrefs.prankcaller)
			addSong('Prank caller', 4, 'bambi', FlxColor.GREEN);
		...
		bg = new FlxSprite().loadGraphic(Paths.image('freeplay/Extra'));
	...
	{
		songs.push(new SongMetadataExtra(songName, weekNum, songCharacter, color));
	}
