...
class FreeplayMeniState extends MusicBeatState
{
	var songs:Array<SongMetadataMeni> = [];
	...
	{
		addSong('Masker', 4, 'menimated', FlxColor.GRAY);
		addSong('Not on my watch', 4, 'menimated', FlxColor.GRAY);
		addSong('Right on', 4, 'hinata', FlxColor.YELLOW);
		addSong('Anger issue', 4, 'menimatedanger', FlxColor.GRAY);
		...
		bg = new FlxSprite().loadGraphic(Paths.image('menuMeni'));
	...
	{
		songs.push(new SongMetadataMeni(songName, weekNum, songCharacter, color));
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.UI_RIGHT_P)
		{
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
			FlxG.switchState(new FreeplayDNBState());
		    });
			FlxG.sound.music.fadeOut(0.8, 0);
			new FlxTimer().start(0.8, function(tmr:FlxTimer)
			{
