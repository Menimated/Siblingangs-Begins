...
class GameState extends MusicBeatState
{
	...
	var menuDescs:Array<String> = [
		'Play through by sing against Kazashi and Zuki!',
		'This is a remake version of VS Menimated, You can also singing against Meni and Hinata!',
		'???'
	];
	...
	override function create()
	{
		...
		if(Highscore.getScore('Anger-issue', 2) != 0){
			ClientPrefs.weekMeni = true;
			ClientPrefs.saveSettings();
		}
		if(ClientPrefs.weekMeni){
			optionShit.insert(2,"game3");
			optionShit.remove("weeklocked");
			menuDescs.insert(2,"Sing against Dave and Bambi!");
			menuDescs.remove("???");
		}

		/*if(Highscore.getScore('Freaking-out', 2) != 0){
			MusicBeatState.switchState(new EndCreditsState());
			FlxG.sound.playMusic(Paths.music('nothing'), 0.7);
			ClientPrefs.EndingCredits = true;
					...
					menuItems.forEach(function(spr:FlxSprite)
					{
						...
						else
						{
							FlxFlicker.flicker(spr, 1, 0.06, false, false, function(flick:FlxFlicker)
							{
								...
								{
									case 'game':
										MusicBeatState.switchState(new WeekSelectStateComplete());
									case 'game2':
									    loadWeekMeni();
	...
	function loadWeekMeni()
		{
			...
			var leWeek:Array<Dynamic> = WeekData.weeksLoaded.get(WeekData.weeksList[5]).songs;
	...
	function loadWeekDNB()
		{
			...
			var songArray:Array<String> = [];
			var leWeek:Array<Dynamic> = WeekData.weeksLoaded.get(WeekData.weeksList[6]).songs;
