...
class HTitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			FlxG.sound.playMusic(Paths.music('creepy'), 0.7, true);
			Application.current.window.title = "EXPUNGED TAKEOVER";
		}
		...
		logoBl = new FlxSprite(250, 90);
		logoBl.frames = Paths.getSparrowAtlas('logoBumpin2');
		logoBl.antialiasing = ClientPrefs.globalAntialiasing;
		logoBl.animation.addByPrefix('bump', 'logo bumpin', 94);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if (pressedEnter)
			{
				...
				FlxG.sound.play(Paths.sound('startTitleH'), 0.7);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
					case 0:
						createCoolText(['TAKING OVER BY'], -40);
						addMoreText('EXPUNGED', -40);
					case 1:
						deleteCoolText();
					    createCoolText(['TAKING OVER BY'], -40);
						addMoreText('EXPUNGED', -40);
