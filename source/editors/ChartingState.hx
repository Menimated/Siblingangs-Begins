...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
        ...
        /*
		if(_song.song.toLowerCase() == "unsurvivable" || _song.song.toLowerCase() == "cheaters-never-win" || _song.song.toLowerCase() == "defeat" || _song.song.toLowerCase() == "expurgation",) {
			FlxG.switchState(new ExpungedCaught());
		}
        */

		switch(_song.song.toLowerCase()) {
			case 'unsurvivable' | 'cheaters-never-win' | 'defeat' | 'expurgation':
				FlxG.switchState(new ExpungedCaught());
			case 'what':
				CoolUtil.browserLoad('https://www.youtube.com/watch?v=ZjNpmBaR3LE');
			    Sys.exit(0);
		}
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				//FlxG.switchState(new CheaterAlert());
			}
