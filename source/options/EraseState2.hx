...
class EraseState2 extends MusicBeatState
{
	...
	function openSelectedSubstate(label:String) {
		switch(label) {
			case 'Yes':
                MusicBeatState.switchState(new options.EraseState3());
	...
	override function create() {
		...
		if (!FlxG.sound.music.playing)
			{	
				FlxG.sound.playMusic(Paths.music('alarm'), 0.7);
				FlxG.sound.music.time = 0;
				Conductor.changeBPM(102);
			}

        menuText = new FlxText(0, 0, FlxG.width, "Wait a minute, You gonna lose all of your achievements, 
		including your Story progress and scores! Are you sure you want to delete it?,", 20);
